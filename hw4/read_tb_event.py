...
##############################################################
 ...
 # Last Modified : 廿十八年五月十八日 (週五) 廿二時34分57秒
 # Created By : SL Chung
##############################################################
import os
import sys
...
sys.stdout.flush()
output_path = os.path.join(sys.argv[2], 'fig1_2.jpg')
...
plt.ylabel('Loss')
plt.grid()
plt.plot(step_DR, DR, '-' ,label='Real', c=[1,0,0] )
plt.plot(step_DF, DF, '-' ,label='Fake', c=[0,1,0] )
plt.legend()
...
sys.stdout.flush()
output_path = os.path.join(sys.argv[2], 'fig2_2.jpg')
plt.savefig(output_path)
...
plt.ylabel('Loss')
plt.grid()
plt.plot(step_CR, CR, '-' ,label='Real', c=[1,0,0] )
plt.plot(step_CF, CF, '-' ,label='Fake', c=[0,0,1] )
plt.legend()
print('Output: ACGAN figs'  )
sys.stdout.flush()
output_path = os.path.join(sys.argv[2], 'fig3_2.jpg')
plt.savefig(output_path)
